...
/* Client-shape card generated by the AI for a destination NOT in the fixed
   catalog (Parigi, Grecia, Praga…). Rendered directly by the home FeedCard grid.
   Prices are declared AI estimates (tag 'STIMA AI'), never fake locked/struck
   feed prices — originalPrice is null so no discount/strike-through is shown. */
...
export type AiTripResult =
  ...
  | { mode: 'destination'; destination: string; country: string; summary: string; flights: DestinationCard[]; hotels: DestinationCard[] }
  | {
      mode: 'filter';
      summary: string;
      flightIds: string[];
...
/* Deep links for AI-generated destination cards: a bare destination has no
...
PASSO 1 — DECIDI LA MODALITÀ (questa decisione viene PRIMA di ogni altra considerazione). Scegli UNA tra "trip", "destination", "filter":
- Modalità "trip" se la richiesta contiene ALMENO UNO di questi elementi: date o periodo, durata del viaggio, numero di persone, budget, città di partenza, oppure chiede esplicitamente di ORGANIZZARE un viaggio/vacanza/pacchetto/itinerario.
...
- Modalità "destination" se la richiesta è SOLO un luogo o un mood di viaggio SENZA date/durata/persone/budget, e quel luogo NON è tra le destinazioni del catalogo fornito.
  Esempi che DEVONO dare "destination": "Parigi" · "Grecia" · "Praga" · "un posto di mare" · "weekend romantico" · "montagna a settembre" (senza altri dettagli). In questa modalità genera opzioni reali di voli+hotel per quella destinazione dalla tua conoscenza del mondo, IGNORANDO il catalogo.
- Modalità "filter" SOLO quando la parola chiave corrisponde chiaramente a una destinazione GIÀ PRESENTE nel catalogo fornito (usane gli id). Esempi: "bali", "tokyo" se sono nel catalogo · "voli economici" · "hotel con piscina".
IN DUBBIO tra "destination" e "filter": se il luogo NON è nel catalogo → "destination".
IMPORTANTE: il catalogo fornito serve SOLO per la modalità "filter". In "trip" e "destination" IGNORALO COMPLETAMENTE. Un budget stretto NON è mai un motivo per scegliere "filter".
...
async function normalizeDestination(parsed: any, lang?: string): Promise<AiTripResult> {
  ...
  return { mode: 'destination', destination, country, summary: String(parsed.summary || ''), flights, hotels };
}
